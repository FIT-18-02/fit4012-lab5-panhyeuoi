...
int main(int argc, char* argv[]) {
    if (argc < 4) {
        cout << "Usage: ./cbc_decrypt <input> <output> <keyfile>" << endl;
        return 1;
    }
    ...
    // 1. Đọc file nhị phân (Cách chuẩn cho AES)
    ifstream infile(inputFileName, ios::binary | ios::ate);
    if (!infile.is_open()) return 1;
    
    int n = infile.tellg(); // Lấy độ dài thật của file
    infile.seekg(0, ios::beg);
    unsigned char * encryptedMessage = new unsigned char[n];
    infile.read((char*)encryptedMessage, n);
    infile.close();

    // 2. Đọc Key (Giả sử bạn đã có hàm readKey)
    unsigned char key[16]; 
    // ... code đọc key từ keyFileName của bạn ở đây ...

    unsigned char * decryptedMessage = new unsigned char[n];
    unsigned char expandedKey[176];
    KeyExpansion(key, expandedKey);

    // --- LOGIC GIẢI MÃ CBC BẠN ĐÃ VIẾT (RẤT TỐT) ---
    unsigned char iv[16] = {0}; 
    unsigned char previous_block[16];
    memcpy(previous_block, iv, 16); 

    for (int i = 0; i < n; i += 16) {
        unsigned char current_ciphertext[16];
        unsigned char decrypted_block[16];
        unsigned char final_plaintext[16];

        memcpy(current_ciphertext, encryptedMessage + i, 16);
        AESDecrypt(current_ciphertext, expandedKey, decrypted_block);
        xor_blocks(decrypted_block, previous_block, final_plaintext);
        
        memcpy(previous_block, current_ciphertext, 16);
        memcpy(decryptedMessage + i, final_plaintext, 16);
    }

    // 3. Ghi kết quả ra file
    ofstream outfile(outputFileName, ios::binary);
    if (outfile.is_open()) {
        outfile.write((char*)decryptedMessage, n);
        outfile.close();
        cout << "Giai ma thanh cong! File da tao: " << outputFileName << endl;
    }
